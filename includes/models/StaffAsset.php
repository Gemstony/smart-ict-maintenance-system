...
public function getAll($filters = [])
{
    ...
    JOIN users staff ON staff.department = a.location
    WHERE staff.user_id = ?
    ";

    $params = [];
    $params[] = $_SESSION['user_id']; // IMPORTANT: bind the first ?
    ...
    {
        $stmt = $this->db->prepare("SELECT * FROM assets WHERE asset_id = ? AND location = (SELECT department FROM users WHERE user_id = ?)");
